...
const Person = (props) => {
  return (
    <Grid container>
      ...
      <Grid item xs={8}>
        <div style={{ fontSize: "1.2em" }}>{props.name}</div>
        <div style={{ fontSize: "1.2em" }}>{props.email}</div>
      </Grid>
    </Grid>
  );
};
...
const Console = (props) => {
  ...
  const addClick = () => {
    ...
    if (amount <= 0) showToast("Invalid cash in amount");
    else if (cause.length === 0) showToast("Please enter cause of click");
    else {
      setAddLoading(true);
      // console.log(Date.now());
  ...
  const withdrawClick = () => {
    ...
    if (amount <= 0) showToast("Invalid withdraw amount");
    else if (cause.length === 0) showToast("Please enter cause of withdrawal");
    else {
      ...
      <center>
        <div style={{ marginTop: "20px", fontSize: "2.5em", color: "#0090ff" }}>
          {balance === null ? (
            ...
            <div style={{ color: "#aa0000" }}>{balance}$</div>
          )}
        </div>
      </center>
    ...
    <div className={classes.root}>
      ...
      <Dialog open={addDialog}>
        ...
        <DialogActions>
          ...
          <Button disabled={addLoading} onClick={addClick} color={"primary"}>
            Cash In
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog open={withdrawDialog}>
        {withdrawLoading ? <LinearProgress /> : <div />}
      ...
      <CssBaseline />
      <AppBar
        style={{ backgroundColor: "#0090ff", color: "#ffffff" }}
